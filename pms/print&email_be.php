...
if(isset($_POST['print_int_reg_details']))
{
...
$sql="SELECT * FROM invoice where folioId= $_SESSION[folio]" ;
          ...
          {
            $final_debit+=$row['debit'];
            $final_credit+=$row['credit'];
          }
$sql="select * from  reservation where folio_id=$_SESSION[folio]";
$res=mysqli_query($con,$sql);
while($row=mysqli_fetch_array($res))
{
  $final_credit+=$row['received_amount'];
  $final_debit+=$row['total'];
}
$final_balance= $final_debit - $final_credit;
/*if($final_credit>=$final_debit)
{
  $final_balance=$final_credit - $final_debit;
}
else
{
   $final_balance=$final_debit - $final_debit;
}
*/
...
if(isset($_POST['print_reg_details_dom']))
{
	$query="SELECT * FROM setting_invoice";
$result=mysqli_query($con,$query) or die(mysqli_error($con));
$final_row=mysqli_fetch_assoc($result);
...
echo"<div align='center'><label>Registration Card</label><br>";
echo"<label>Domestic Guest</label></div>";
echo"<div>
