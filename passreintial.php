...
<body>
	...
	<div id="passr" class="card r">
		...
		<p>Your password has been successfully changed! Use your new password to <a href="http://localhost/v1hotel/hotellogin.php">login</a></p>
	</div>
...
<?php
	if (isset($_POST['pfcsubmit'])) {
		...
		if (isset($_GET['fgpass']) && isset($_GET['fgpassu'])) {
			if ($newpass !== $confpass) {
		      echo "<script>alert('Passwords do no match')</script>";
		    }else{
		  	$newpass = password_hash($newpass, PASSWORD_DEFAULT);//encrypt the password before saving in the database
		  	$querys = "UPDATE user".$_GET['fgpassu']." SET password = '$newpass' WHERE password = '$_GET[fgpass]'";
