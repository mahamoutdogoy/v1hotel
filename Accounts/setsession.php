<?php
	...
	$query11=mysqli_query($conn,"select * from rooms_details where property_id='".$_SESSION['user']['hotelid']."' and room_id=$_GET[rid]") or die(mysqli_error($conn));
	 if(mysqli_num_rows($query11)>0)
     {     
     	$query13=mysqli_query($conn,"select * from rooms_tax where  property_id='".$_SESSION['user']['hotelid']."' and room_id=$_GET[rid] and meal_plan='EP'") or die(mysqli_error($conn));
		if(mysqli_num_rows($query13)>0)
		{
     	...
     	$query12=mysqli_query($conn,"select * from rooms_tariff where  property_id='".$_SESSION['user']['hotelid']."' and room_id=$_GET[rid] and meal_plan='EP' ") or die(mysqli_error($conn));
        ...
        echo "<div class='col-md-6 roomtype'><caption>".$row11['roomType']."</caption>";   
	 ...
	 for($i=0;$i<$_GET['noroom'];$i++)
              {
                ...
                <option value="0">0</option>';
                 for($j=1;$j<=$row11['max_child_occupancy'];$j++)
                {
                  $htmlstr.='<option value="'.$j.'">'.$j.'</option>';
                }
