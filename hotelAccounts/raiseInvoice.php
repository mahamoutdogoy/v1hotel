...
<body>
	...
	<?php
	 include_once("../mytravalyAdmin/myheader.php"); 
	include_once("../mytravalyAdmin/mtsidebar.php"); 	
	?>
			...
			<div class="col-md-9 col-lg-10 ">
				<form action="raiseInvoice_code.php" method="post">
				...
				<?php 
				 ...
				 if(mysqli_num_rows($query)>0){
				 	...
				 	if(strcasecmp(trim($request_status), "Paid")==0)
				 	{
				 		...
				 		echo '<div class="row  ml-1 p-3" style="color:green;border-radius:10px;border:solid 2px green"><i class="far fa-check-circle fa-2x pt-2" ></i><h3 class="ml-5 pt-2">Request Already Placed...</h3>  </div>';
				 	}
	...
	<div>
        ...
        <thead>
           <tr class="bg-warning">                
                <th style="border-radius: 10px 0px 0px 10px ">Room Id</th>
                <th>Price</th>
                <th>Tax/GST</th>
                <th>Total</th>
                <th>Payable</th>
        	...
        	$total=0;
        		if(mysqli_num_rows($query))
        		{
        			while ($row=mysqli_fetch_array($query)) {
        				echo '<tr><td>'.$row['room_id']."/".$row['inventory_id'].'</td>
        				<td>'.$row['price'].'</td>
        				<td>'.$row['tax'].'</td>
        				<td>'.$row['total'].'</td>
	...
	<div>
		<input type="submit" class="btn btn-success float-right" name="raise_req" value="Request" <?php 
		if($request_status!= 'Not Yet Initiated') echo "disabled"; ?> >
