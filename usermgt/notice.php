...
<head>
  ...
  <title>Mytravaly Notice list</title>
...
<body>
  ...
  <div class="modal-dialog modal-dialog-centered">
    ...
    <div class="row">
      <?php
        $sel_sql = "SELECT * FROM notices WHERE nuserid = '$loginid' ORDER BY notid DESC";
        $run =mysqli_query($con, $sel_sql);
        while ($rows = mysqli_fetch_assoc($run)) {
          # code... 
           echo "<div class='col-md-6'><div class='card ml-1 mb-2'>
            <div class='card-header'>
              <h5><i class='fas fa-info'></i> $rows[notname] <form action='' method='post' class='float-right'><input type='hidden' name='delid' value='$rows[notid]'><input type='submit' value='X' class='btn btn-warning btn-lg' onclick = 'return del_conf();'></form></h5>
            </div>
            <div class='card-body'>
              <p>$rows[notcontent]</p>
            </div>
...
<?php
  if(isset($_POST['createNotice'])){
    ...
    $ins_sql = "INSERT INTO notices (nuserid, notname, notcontent) VALUES ('$loginid', '$title', '$content')";
...
<?php
  if(isset($_POST['delid'])){
    $del_sql = "DELETE FROM notices WHERE notid = '$_POST[delid]' AND nuserid = '$loginid'";
