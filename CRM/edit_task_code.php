<?php
  ...
  if(!isset($_SESSION['user']))
  {
	...
	if(isset($_GET'taskUpdate'])  ) 
	{
		//updating  task Details
		$task=trim(mysqli_real_escape_string($conn, strip_tags($_GET'task'])));
		...
		$status=mysqli_real_escape_string($conn, strip_tags($_GET'status']));
		$description=trim(mysqli_real_escape_string($conn, strip_tags($_GET'description'])));
		$remarks=mysqli_real_escape_string($conn, strip_tags($_GET'remarks']));
		$duedate=mysqli_real_escape_string($conn, strip_tags($_GET'duedate']));
