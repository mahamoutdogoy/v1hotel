...
<?php
   ...
   //echo $session_user;
	...
	/* REGIONS */
	$result = mysqli_query($con, 'SELECT * FROM `rooms_tariff`  group by p_r_id ');
	$s=array(array());
	$update=array(array());
	$s1=array();
	$s2=array();
	...
	while($row = mysqli_fetch_assoc($result))
	{
		...
		$result2=mysqli_query($con,"select * from rooms_detail where property_id='$row[property_id]' and room_id='$row[room_id]'");
		...
		$resultx=mysqli_query($con,"select * from rooms_offers where p_r_id='$row[p_r_id]'");
	...
	$result = mysqli_query($con, 'SELECT p_r_id,start_date,end_date,user_id FROM `rooms_offers` 
	...
	$resultXX = mysqli_query($con, 'SELECT p_r_id,start_date,end_date,user_id FROM `rooms_offers` 
							WHERE YEAR(`start_date`) = YEAR("'.$requYMD.'")
    ...
    {	
		...
		$output = '
	<table id="table_'.$countryCode.'" border="5px" class="bordered table table2" >
	<tr>
	...
	$j=4;
        ...
        {
        	$v=1;
        	$output .= '<tbody><tr class="parent"> <td style="font-size:13px"><i class="fa fa-chevron-down"></i><input class="fa fa-chevron-down" type="button" onclick="myFunction(this)" id="btnhide" value="Tariff" data-trid="tr'.$id.'"> '.$regionNames[$id]."-".$meal_plan[$id].'</a></td></tr>';
        	while($max_occupancy[$id]>0)
		{
			...
			if($v==1)
			{
			$output .= '<tr class="cchild" style="background:white;color:blue" > <td style="text-align:center">&nbsp; '.$regionNames[$id]."-".$meal_plan[$id]."-P".$v.'</a></td>';
			}
			else
			{
				$output .= '<tr class="cchild" style="background:white;color:blue" > <td style="text-align:center" > '.$regionNames[$id]."-".$meal_plan[$id]."-P".$v.'</a></td>';
...
<body class="holidaysm">
	...
	{
	$mt_property_id=$_GET['property_id'];
	}
	if($mt_property_id!='')
	{
		
		$query=mysqli_query($con,"select * from rooms_tariff right join propertydetails on rooms_tariff.property_id=propertydetails.property_id where rooms_tariff.property_id='$mt_property_id' group by propertydetails.property_id");
	}
	else
	{
	...
	$query=mysqli_query($con,"select * from rooms_tariff right join propertydetails on rooms_tariff.property_id=propertydetails.property_id where propertydetails.user_id='$session_user'  group by propertydetails.property_id");
	}
	$o=10;
	...
	while ($row1=mysqli_fetch_array($query1) ){*/
		...
		$pid=$row[25];
		$query3=mysqli_query($con,"select * from propertydetails where property_id='$pid'");
		...
		if($row['status']==1)
		{
			...
			if($row[1]!=NULL)
			{
				...
				$res=mysqli_query($con,"select property_score from property_score where property_id='$pid'");
				$res1=mysqli_fetch_array($res);
				$case=$res1[0];
				switch ($case) {
				    case 1:
				        	 echo '<b>Property Score</b>&emsp;<div id="progressbar" style="display: inline-block;width: 400px">
				    ...
				    case 9:
				        ...
				        break;
				   	default:
					   	echo '<b>Property Score</b>&emsp;<div id="progressbar" style="display: inline-block;width: 400px;height:25px">&emsp;0%
			...
			{
				echo '<h1> '.$row3['property_name'].' </h1> You have not created any rooms yet please <a href="rooms.php">create now.</a>';
		...
		{
			echo '<h1> '.$row3['property_name'].' </h1> Property is under review, shortly it wil be live, mean while you may uplaod the rooms, tariff, photos, amenites, policy, bankdetails and documents.';
		}
