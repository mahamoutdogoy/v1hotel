				...
				<div class="container">
					...
					<div class="card">
						<table class="table table-hover shopping-cart-wrap">
							...
							$query_type = "select * from rooms_detail where  property_id=".$_REQUEST['property_id']."  group by room_id";
							...
							while($row_room = mysqli_fetch_array($result_type)){
								
								
								$reservation = "select count(*) AS count  from rooms_inventory where property_id=".$_REQUEST['property_id']." and room_id='$row_room[room_id]' and room_sub_id not in (select inventory_id from reservation where property_id=".$_REQUEST['property_id']." and room_id='$row_room[room_id]' and (check_in <= '$_SESSION[check_out]') and (check_out >= '$_SESSION[check_in]') and( status=1 or status=0 ) )";

							$reservation_result = mysqli_query($con,$reservation);
							
							$query_room = "SELECT * FROM meal_plan WHERE property_id = $_REQUEST[property_id] AND room_id=$row_room[room_id]";
							$result_room = mysqli_query($con,$query_room);
							if(mysqli_num_rows($reservation_result)>0){
							$meal_pl = mysqli_fetch_array($result_room);
							$row_reservation = mysqli_fetch_array($reservation_result);
							$_SESSION['room_type']= $row_room['room_name'];

								$_SESSION['single_price'] = $meal_pl['singleprice'];
								
								$_SESSION['meal_plan']    = $meal_pl['meal_plan'];


								
								
									$adults1 = $_SESSION['people'];
									$room_price = $meal_pl['singleprice'];
								...
								if($_SESSION['people']==1){
									...
									elseif ($_SESSION['rooms']==2) {
										$room_price = $meal_pl['singleprice'] * 2;
										$_SESSION['full_price'] = $room_price;
									}elseif ($_SESSION['rooms']==3) {
										$room_price = $meal_pl['singleprice'] * 3;
										$_SESSION['full_price'] = $room_price;
									}elseif($_SESSION['rooms']==4){
										$room_price = $meal_pl['singleprice'] * 4;
										$_SESSION['full_price'] = $room_price;
									}
									...
									$room_price=0;
									if($_SESSION['rooms']==1){
										$room_price = $meal_pl['doubleprice'];
										$_SESSION['full_price'] = $room_price;
									}
									elseif ($_SESSION['rooms']==2) {
										$room_price = $meal_pl['doubleprice'] * 2;
										$_SESSION['full_price'] = $room_price;
									}elseif ($_SESSION['rooms']==3) {
										$room_price = $meal_pl['doubleprice'] * 3;
										$_SESSION['full_price'] = $room_price;
									}elseif($_SESSION['rooms']==4){
										$room_price = $meal_pl['doubleprice'] * 4;
										$_SESSION['full_price'] = $room_price;
									}
									...
									$room_price=0;
									if($_SESSION['rooms']==1){
										$room_price = $meal_pl['tripleprice'];
										$_SESSION['full_price'] = $room_price;
									}
									elseif ($_SESSION['rooms']==2) {
										$room_price = $meal_pl['tripleprice'] * 2;
										$_SESSION['full_price'] = $room_price;
									}elseif ($_SESSION['rooms']==3) {
										$room_price = $meal_pl['tripleprice'] * 3;
										$_SESSION['full_price'] = $room_price;
									}elseif($_SESSION['rooms']==4){
										$room_price = $meal_pl['tripleprice'] * 4;
										$_SESSION['full_price'] = $room_price;
									}
								}
								elseif ($_SESSION['people']==4) {
									$room_price=0;
									if($_SESSION['rooms']==1){
										$room_price = $meal_pl['person4price'];
										$_SESSION['full_price'] = $room_price;
									}
									elseif ($_SESSION['rooms']==2) {
										$room_price = $meal_pl['person4price'] * 2;
										$_SESSION['full_price'] = $room_price;
									}elseif ($_SESSION['rooms']==3) {
										$room_price = $meal_pl['person4price'] * 3;
										$_SESSION['full_price'] = $room_price;
									}elseif($_SESSION['rooms']==4){
										$room_price = $meal_pl['person4price'] * 4;
										$_SESSION['full_price'] = $room_price;
									}
								}
							}
								...
								<h6 class="title text-truncate">'.$row1["property_name"].','.$row1["address"].$row_room['room_id'].$row_reservation["count"].'</h6>
								<dl class="param param-inline small">
								<dt><h4>'.$row1["property_name"].$star.'</h4></dt>
